...
def mock_collection():
    ...
    collection.query.return_value = {
        "ids": [["doc_art_1_p_0", "doc_art_2_p_0"]],
...
def test_add_chunks_passes_chunk_uid_as_id(store, sample_chunks, sample_embeddings):
    input = AddChunksInput(chunks=sample_chunks, embeddings=sample_embeddings)
    store.add_chunks(input)
    call_kwargs = store.collection.upsert.call_args.kwargs
    assert call_kwargs["ids"] == ["doc_art_1_p_0", "doc_art_2_p_0"]
...
def test_add_chunks_passes_correct_metadatas(store, sample_chunks, sample_embeddings):
    ...
    assert call_kwargs["metadatas"][0]["type"] == "article"
    assert call_kwargs["metadatas"][0]["article_number"] == "1"
...
def test_search_result_fields(store):
    input = SearchInput(query_embedding=[0.1, 0.2, 0.3], top_k=2)
    results = store.search(input)
    assert results[0].chunk_uid == "doc_art_1_p_0"
    assert results[0].content == "content 1"
    assert results[0].distance == 0.1
    assert results[1].chunk_uid == "doc_art_2_p_0"
...
def test_search_invalid_top_k_raises(store):
    with pytest.raises(ValueError):
        SearchInput(query_embedding=[0.1, 0.2, 0.3], top_k=0)
...
def test_real_chroma_store():
    ...
    assert results[0].chunk_uid == "real_chunk_1"
